...
def test_environment_build():
    plugin_a = Plugin(name="a", version="1", path=Path("/a/zorro-plugin.json"), env={
        "PATH": PluginEnv(append=[Path("./foo/"), Path("./bar/")], prepend=[Path("./baz/")]),
        "PYTHONPATH": PluginEnv(append=[Path("./code")]),
        "DEBUG": PluginEnv(set="info"),
        "TOTO": PluginEnv(set="toto")
        })
    plugin_b = Plugin(name="b", version="1", path=Path("/b/zorro-plugin.json"), env={
        "PATH": PluginEnv(append=[Path("./foo/")], prepend=[Path("./baz/")]),
        "PYTHONPATH": PluginEnv(prepend=[Path("./code")]),
        "DEBUG": PluginEnv(set="debug")
        })
    plugin_c = Plugin(name="a", version="1", path=Path("/c/zorro-plugin.json"), env={
        "PATH": PluginEnv(append=[Path("./foo/")], prepend=[Path("./baz/"), Path("./bar/")]),
